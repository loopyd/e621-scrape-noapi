...
def ExistingWritableDir(dir_name: str) -> str:
    ...
    if (
        ...
        raise argparse.ArgumentTypeError(f"{dir_name} is not an existing writable directory.")
...
def parse_args(args: str) -> AppConfig:
    parser = argparse.ArgumentParser(prog="e621-noapi-cli", description='Yiffy Diffusion NoAPI scraper for e621.net by the furry diffusion community.', add_help=True)
    parser.add_argument('csv-dir', type=str, help='The directory to store downloaded e621 CSV data (for our offline API).', dest='csv_dir')
    parser.add_argument('refresh-csv', type=bool, help='Clean and redownload the e621 csv database.', dest='refresh_csv', action="store_true", default=False)
    parser.add_argument('search', type=List, help='Specify your search query, you can use exact e6 search query format without tag limits thanks to our custom parser', dest='search_query')
    parser.add_argument('samples', type=int, help='Number of images (samples) to download', dest='num_samples')
    parser.add_argument('sample-dir', type=str, help='The base directory to store downloaded sample data to', dest='sample_dir')
    parser.add_argument('store-metadata', type=bool, help='Dump metadata information next to each file', action='store_true', dest='store_metadata', default=False)
    parser.add_argument('build-tokendb', type=bool, help='Build token database file (.txt) for stable diffusion embedding training.', action='store_true', dest='build_tokendb', default=False)
...
def main(config: AppConfig):
    ...
    posts_df = pd.read_csv(f"{config.csv_dir}/posts.csv.gz", sep=",", quotechar='"',
                           converters={"tag_string": lambda x: set(x.split(' '))}, compression="gzip")
    posts_df.rename(columns={"tag_string": "tags"}, inplace=True)
    logging.info(f"Loading post database with {len(posts_df)} posts took {time.time() - time_before:.1f}s")
    ...
    posts_df = filter_posts_by_tags(posts_df, None, {"young", "cub", "loli", "shota", "gore", "scat"})
    logging.info(
    ...
    num_explicit_posts = len(posts_df.query("rating == 'e'"))  # Note the lack of inplace=True to avoid modifying it
    ...
    posts_df.sort_values("fav_count", axis=0, ascending=False, inplace=True)  # Optionally pass inplace=False...
    logging.info(f"The highest fav_count post in the dataset has {posts_df['fav_count'].values[0]} favorites")
    ...
    logging.info(f"There are {len(species_tags)} different species tags across the original unfiltered dataset")

    post_species_tags = [t for t in posts_df["tags"].values[0] if t in species_tags["name"].values]
    ...
    rating_e_order_score = posts_df.query("rating == 'e'").sort_values("score", axis=0, ascending=False)

    # Rough equivalent of "rating:e order:score mr._wolf_(the_bad_guys) wolf_o'donnell"
    wolf_and_mr_wolf = filter_posts_by_tags(rating_e_order_score, {"mr._wolf_(the_bad_guys)", "wolf_o'donnell"})
    # "rating:e order:score ~mr._wolf_(the_bad_guys) ~wolf_o'donnell"
    wolf_or_mr_wolf = filter_posts_by_tags(rating_e_order_score, None, None,
                                           {"mr._wolf_(the_bad_guys)", "wolf_o'donnell"})
    # "rating:e order:score ~mr._wolf_(the_bad_guys) ~wolf_o'donnell -fox_mccloud"
    wolf_or_mr_wolf_and_not_fox = filter_posts_by_tags(rating_e_order_score, None, {"fox_mccloud"},
                                                       {"mr._wolf_(the_bad_guys)", "wolf_o'donnell"})
    ...
    logging.info(f"'rating:e order:score mr._wolf_(the_bad_guys) wolf_o'donnell': {len(wolf_and_mr_wolf)} results")
    logging.info(f"'rating:e order:score ~mr._wolf_(the_bad_guys) ~wolf_o'donnell': {len(wolf_or_mr_wolf)} results")
    logging.info(f"'rating:e order:score ~mr._wolf_(the_bad_guys) ~wolf_o'donnell -fox_mccloud': "
